        ...
        <main className="unidades__main">
          ...
          <ol className="unidades__lista" aria-label="Lista de unidades">
            {unidades.map((unidade) => {
              // 💡 CORREÇÃO AQUI: Garante um fallback seguro caso o status venha indefinido ou não mapeado
              const statusInfo = STATUS_CONFIG[unidade.status] || STATUS_CONFIG['disponivel'];
              ...
              return (
                <li key={unidade.id}>
                  <article
                    ...
                    // 💡 CORREÇÃO AQUI: Adicionado ?. e fallback para o aria-label não quebrar
                    aria-label={`Unidade ${unidade.numero}: ${unidade.titulo}. Status: ${statusInfo?.label || 'Disponível'}.${unidade.progresso > 0 ? ` Progresso: ${unidade.progresso}%.` : ''}`}
                  ...
                  >
                    ...
                    <div className="unidades__card-conteudo">
                      <div className="unidades__card-topo">
                        ...
                        {/* 💡 CORREÇÃO AQUI: Evita ler propriedade de undefined na classe e no label */}
                        <span className={`unidades__card-status unidades__card-status--${statusInfo?.classe || 'disponivel'}`}>
